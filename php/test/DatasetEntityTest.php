...
class DatasetEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = OpenGovernmentPortalSDK::test($seed, null);
        $seen = iterator_to_array($base->Dataset(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = OpenGovernmentPortalConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = OpenGovernmentPortalSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set OPEN_GOVERNMENT_PORTAL_TEST_DATASET_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function dataset_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("OPEN_GOVERNMENT_PORTAL_TEST_DATASET_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["OPEN_GOVERNMENT_PORTAL_TEST_DATASET_ENTID"]);
    if ($idmap_resolved === null) {
        $idmap_resolved = Helpers::to_map($idmap);
    }

    if ($env["OPEN_GOVERNMENT_PORTAL_TEST_LIVE"] === "TRUE") {
        $merged_opts = Vs::merge([
            [
            ],
            $extra ?? [],
        ]);
        $client = new OpenGovernmentPortalSDK(Helpers::to_map($merged_opts));
    }
    ...
    return [
        ...
        "env" => $env,
        "explain" => $env["OPEN_GOVERNMENT_PORTAL_TEST_EXPLAIN"] === "TRUE",
        "live" => $live,
