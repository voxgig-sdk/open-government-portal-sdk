...
// OpenGovernmentPortal SDK pipeline test
//
...
{
    private static function utility(): OpenGovernmentPortalUtility
    {
        return new OpenGovernmentPortalUtility();
    }
    ...
    private static function ctx(array $over = []): OpenGovernmentPortalContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new OpenGovernmentPortalContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new OpenGovernmentPortalOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof OpenGovernmentPortalError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new OpenGovernmentPortalOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = OpenGovernmentPortalMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new OpenGovernmentPortalOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = OpenGovernmentPortalMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new OpenGovernmentPortalOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = OpenGovernmentPortalMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new OpenGovernmentPortalSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = OpenGovernmentPortalMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new OpenGovernmentPortalResponse([]),
            'result' => new OpenGovernmentPortalResult([]),
        ]);
        [, $err] = OpenGovernmentPortalMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new OpenGovernmentPortalSpec([]),
            'response' => null,
            'result' => new OpenGovernmentPortalResult([]),
        ]);
        [, $err] = OpenGovernmentPortalMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new OpenGovernmentPortalSpec(['step' => 's']),
            'response' => new OpenGovernmentPortalResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new OpenGovernmentPortalResult([]),
        ]);
        [, $err] = OpenGovernmentPortalMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new OpenGovernmentPortalSpec(['step' => 's']),
            'response' => new OpenGovernmentPortalResponse(self::resp(200, ['v' => 1])),
            'result' => new OpenGovernmentPortalResult([]),
        ]);
        [, $err] = OpenGovernmentPortalMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new OpenGovernmentPortalSpec(['step' => 's']),
            'response' => new OpenGovernmentPortalResponse(self::resp(200, ['v' => 2])),
            'result' => new OpenGovernmentPortalResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        OpenGovernmentPortalMakeResponse::call($ctx);
    ...
    {
        $preset = new OpenGovernmentPortalResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new OpenGovernmentPortalSpec([]),
            'response' => new OpenGovernmentPortalResponse([]),
            'result' => new OpenGovernmentPortalResult([]),
        ...
        [$got, $err] = OpenGovernmentPortalMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new OpenGovernmentPortalResult([])]);
        [, $err] = OpenGovernmentPortalMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new OpenGovernmentPortalSpec([]), 'result' => null]);
        [, $err] = OpenGovernmentPortalMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new OpenGovernmentPortalSpec(['step' => 's']),
            'result' => new OpenGovernmentPortalResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new OpenGovernmentPortalOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = OpenGovernmentPortalMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new OpenGovernmentPortalSpec(['step' => 's']),
            'result' => new OpenGovernmentPortalResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new OpenGovernmentPortalOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = OpenGovernmentPortalMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new OpenGovernmentPortalResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new OpenGovernmentPortalSpec([]),
            'result' => new OpenGovernmentPortalResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = OpenGovernmentPortalMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = OpenGovernmentPortalMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new OpenGovernmentPortalError('boom', 'boom');
        $utility->fetcher = function (OpenGovernmentPortalContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new OpenGovernmentPortalSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = OpenGovernmentPortalMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (OpenGovernmentPortalContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = OpenGovernmentPortalMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(OpenGovernmentPortalResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        OpenGovernmentPortalMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (OpenGovernmentPortalContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new OpenGovernmentPortalSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = OpenGovernmentPortalMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new OpenGovernmentPortalResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new OpenGovernmentPortalSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = OpenGovernmentPortalMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = OpenGovernmentPortalMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = OpenGovernmentPortalMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new OpenGovernmentPortalResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, OpenGovernmentPortalDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new OpenGovernmentPortalResult(['ok' => false])]);
        $this->expectException(OpenGovernmentPortalError::class);
        OpenGovernmentPortalDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new OpenGovernmentPortalResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', OpenGovernmentPortalMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        OpenGovernmentPortalMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new OpenGovernmentPortalBaseFeature();
        $b = new OpenGovernmentPortalBaseFeature();
        OpenGovernmentPortalFeatureAdd::call($ctx, $a);
        OpenGovernmentPortalFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): OpenGovernmentPortalBaseFeature
    {
        $f = new OpenGovernmentPortalBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        OpenGovernmentPortalFeatureAdd::call($ctx, self::named_feature('a'));
        OpenGovernmentPortalFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        OpenGovernmentPortalFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        OpenGovernmentPortalFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        OpenGovernmentPortalFeatureAdd::call($ctx, $replace);
        ...
        OpenGovernmentPortalFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): OpenGovernmentPortalContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new OpenGovernmentPortalSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = OpenGovernmentPortalPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        OpenGovernmentPortalPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new OpenGovernmentPortalResponse(['headers' => null]),
            'result' => new OpenGovernmentPortalResult([]),
        ]);
        OpenGovernmentPortalResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        OpenGovernmentPortalResultBody::call($ctx);
