...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.OPENGOVERNMENTPORTAL_TEST_LIVE ||
    'TRUE' === process.env.OPENGOVERNMENTPORTAL_TEST_OVERRIDE
  ) {
  ...
  m.OPENGOVERNMENTPORTAL_TEST_EXPLAIN = process.env.OPENGOVERNMENTPORTAL_TEST_EXPLAIN || m.OPENGOVERNMENTPORTAL_TEST_EXPLAIN
