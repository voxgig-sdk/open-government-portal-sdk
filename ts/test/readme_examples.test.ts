...
//     (\`new OpenGovernmentPortalSDK(...)\` / \`OpenGovernmentPortalSDK.test(...)\` become
//     \`OpenGovernmentPortalSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'OpenGovernmentPortalSDK'
...
// rewrite real client construction (\`new OpenGovernmentPortalSDK(...)\` and any
// \`OpenGovernmentPortalSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+OpenGovernmentPortalSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/OpenGovernmentPortalSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
