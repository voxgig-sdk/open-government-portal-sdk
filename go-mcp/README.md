...
[MCP](https://modelcontextprotocol.io) server exposing the OpenGovernmentPortal SDK as
two agent tools — `open-government-portal_list` and `open-government-portal_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/open-government-portal-mcp)
make build

# 2. Provide credentials via the environment
export OPEN_GOVERNMENT_PORTAL_APIKEY=sk_live_xxx
...
```jsonc
// open-government-portal_list: first page of records
{ "entity": "dataset" }
{ "entity": "dataset", "query": { } }

// open-government-portal_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/open-government-portal-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user open-government-portal \
     -- "$PWD"/dist/*/open-government-portal-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `open-government-portal_list` and `open-government-portal_load` tools now appear
   in new sessions. Ask the agent to *"list dataset using open-government-portal"*
   and it calls `open-government-portal_list` with `{"entity":"dataset"}`.
...
```sh
export OPEN_GOVERNMENT_PORTAL_APIKEY=sk_live_xxx            # API key
export OPEN_GOVERNMENT_PORTAL_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `open-government-portal_list` tool
...
### Call the `open-government-portal_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `open-government-portal_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `open-government-portal_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `OPEN_GOVERNMENT_PORTAL_APIKEY` | API key sent with every request. |
| `OPEN_GOVERNMENT_PORTAL_BASE` | Optional override of the API base URL. |
