...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("OPENGOVERNMENTPORTAL_TEST_LIVE") == "TRUE" ||
		os.Getenv("OPENGOVERNMENTPORTAL_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("OPENGOVERNMENTPORTAL_TEST_EXPLAIN"); explain != "" {
