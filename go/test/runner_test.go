...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("OPEN_GOVERNMENT_PORTAL_TEST_LIVE") == "TRUE" ||
		os.Getenv("OPEN_GOVERNMENT_PORTAL_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("OPEN_GOVERNMENT_PORTAL_TEST_EXPLAIN"); explain != "" {
...
type entityTestSetup struct {
	client         *sdk.OpenGovernmentPortalSDK
	data           map[string]any
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.OpenGovernmentPortalSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.OpenGovernmentPortalError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.OpenGovernmentPortalSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.OpenGovernmentPortalError{Msg: msg, Code: code}
}
