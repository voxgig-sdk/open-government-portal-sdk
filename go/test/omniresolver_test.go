...
import (
	...
	omni "github.com/voxgig-sdk/open-government-portal-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.OpenGovernmentPortalError); is {
				out := map[string]any{
					"name":    "OpenGovernmentPortalError",
...
func omniCtx(arg any, client *sdk.OpenGovernmentPortalSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.OpenGovernmentPortalSDK); is {
			client = live
