...
require_relative 'OpenGovernmentPortal_types'
...
class OpenGovernmentPortalSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = OpenGovernmentPortalHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = OpenGovernmentPortalHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, OpenGovernmentPortalFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    OpenGovernmentPortalUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = OpenGovernmentPortalHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = OpenGovernmentPortalHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = OpenGovernmentPortalHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => OpenGovernmentPortalError.new(
        "#{op}_allow",
        "OpenGovernmentPortalSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue OpenGovernmentPortalError => err
    ...
    if fetched.is_a?(Hash)
      status = OpenGovernmentPortalHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = OpenGovernmentPortalError.new(
        "graphql_error", "OpenGovernmentPortalSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = OpenGovernmentPortalSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
