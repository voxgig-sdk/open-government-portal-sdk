# OpenGovernmentPortal SDK test runner
...
class OpenGovernmentPortalTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            OpenGovernmentPortalTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = OpenGovernmentPortalTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = OpenGovernmentPortalTestRunner.getenv("OPEN_GOVERNMENT_PORTAL_TEST_LIVE")
        override = OpenGovernmentPortalTestRunner.getenv("OPEN_GOVERNMENT_PORTAL_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = OpenGovernmentPortalTestRunner.getenv(key)
        ...
        explain = OpenGovernmentPortalTestRunner.getenv("OPEN_GOVERNMENT_PORTAL_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["OPEN_GOVERNMENT_PORTAL_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if OpenGovernmentPortalTestRunner._test_control is not None:
            return OpenGovernmentPortalTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                OpenGovernmentPortalTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            OpenGovernmentPortalTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = OpenGovernmentPortalTestRunner.load_test_control()
...
def load_env_local():
    OpenGovernmentPortalTestRunner.load_env_local()


def env_override(m):
    return OpenGovernmentPortalTestRunner.env_override(m)


def entity_data(v):
    return OpenGovernmentPortalTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return OpenGovernmentPortalTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return OpenGovernmentPortalTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return OpenGovernmentPortalTestRunner.load_test_control()


def live_client_options():
    return OpenGovernmentPortalTestRunner.live_client_options()


def live_delay_ms():
    return OpenGovernmentPortalTestRunner.live_delay_ms()
