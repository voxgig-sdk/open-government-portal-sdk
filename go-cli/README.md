...
boru-driven command-line client **and** interactive REPL for the OpenGovernmentPortal
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/open-government-portal-cli)
make build

# 2. See usage (words, entities, env vars)
./open-government-portal-cli --help
...
./open-government-portal-cli load 1 dataset            # {id:1} shorthand
./open-government-portal-cli load '{id:1}' dataset       # explicit match map

# 5. Override the API base URL for a single call
OPEN_GOVERNMENT_PORTAL_BASE=https://api.example.com ./open-government-portal-cli list dataset

# 6. No arguments -> interactive REPL
./open-government-portal-cli
open-government-portal> list dataset
open-government-portal> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/open-government-portal-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/open-government-portal-cli list dataset
   ```
...
```sh
./open-government-portal-cli load 1 dataset          # scalar shorthand for {id:1}
./open-government-portal-cli load '{id:1}' dataset     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `open-government-portal>`). Each line is
evaluated as its own boru expression:

```text
$ ./open-government-portal-cli
open-government-portal> list dataset
open-government-portal> /help
open-government-portal> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/open-government-portal-cli`. |
...
(`./open-government-portal-cli <expr>`) and interactively (the REPL), and expressions compose the
