...
// OpenGovernmentPortal SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = OpenGovernmentPortalOmni::makeStructRunner(
    ...
    {
        return OpenGovernmentPortalOmni::objmodel(OpenGovernmentPortalOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return OpenGovernmentPortalOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return OpenGovernmentPortalOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(OpenGovernmentPortalOmni::structfix($now, Struct::undef()))
                        !== json_encode(OpenGovernmentPortalOmni::structfix($val, Struct::undef()))) {
