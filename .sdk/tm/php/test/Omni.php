...
//    only - a typed OpenGovernmentPortalContext there would make every ctx
...
final class OpenGovernmentPortalOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new OpenGovernmentPortalContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new OpenGovernmentPortalSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new OpenGovernmentPortalResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new OpenGovernmentPortalError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new OpenGovernmentPortalResponse($clean['response']);
    ...
    public static function err_from_map($m): ?OpenGovernmentPortalError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new OpenGovernmentPortalError(is_string($code) ? $code : '', $msg);
    }
