...
// OpenGovernmentPortal SDK feature test
//
...
class FtCtrl extends OpenGovernmentPortalControl
{
...
class FtClient
{
    ...
    private OpenGovernmentPortalUtility $utility;

    public function __construct(OpenGovernmentPortalUtility $utility, array $options)
    {
    ...
    public function get_utility(): OpenGovernmentPortalUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (OpenGovernmentPortalContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public OpenGovernmentPortalContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (OpenGovernmentPortalContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = OpenGovernmentPortalConfig::make_feature($name);
    ...
    private static function buildUrl(OpenGovernmentPortalSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof OpenGovernmentPortalError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new OpenGovernmentPortalResponse($fetched);
        }
    ...
    private function fail(OpenGovernmentPortalContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (OpenGovernmentPortalContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
