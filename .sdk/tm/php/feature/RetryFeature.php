...
// OpenGovernmentPortal SDK retry feature
...
class OpenGovernmentPortalRetryFeature extends OpenGovernmentPortalBaseFeature
{
    ...
    public function init(OpenGovernmentPortalContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (OpenGovernmentPortalContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(OpenGovernmentPortalContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(OpenGovernmentPortalContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
