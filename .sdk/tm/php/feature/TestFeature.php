...
// OpenGovernmentPortal SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class OpenGovernmentPortalTestFeature extends OpenGovernmentPortalBaseFeature
{
    ...
    public function init(OpenGovernmentPortalContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (OpenGovernmentPortalContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = OpenGovernmentPortalParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (OpenGovernmentPortalContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(OpenGovernmentPortalContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = OpenGovernmentPortalParam::call($ctx, $k);
