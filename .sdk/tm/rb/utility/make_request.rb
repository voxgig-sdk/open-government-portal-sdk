...
module OpenGovernmentPortalUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = OpenGovernmentPortalResponse.new({})
    ...
    if fetch_err
      ...
      response = OpenGovernmentPortalResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = OpenGovernmentPortalResponse.new(fetched)
    else
