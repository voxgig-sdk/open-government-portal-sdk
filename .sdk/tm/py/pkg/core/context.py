...
from projectname_sdk.core.control import OpenGovernmentPortalControl
...
class OpenGovernmentPortalContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = OpenGovernmentPortalControl()
        ...
        if isinstance(sp, OpenGovernmentPortalSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, OpenGovernmentPortalResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, OpenGovernmentPortalResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return OpenGovernmentPortalOperation({})
        ...
        op = OpenGovernmentPortalOperation({
            "entity": entname,
            "name": opname,
            "input": inpt,
            "points": points,
    ...
    def make_error(self, code, msg):
        return OpenGovernmentPortalError(code, msg, self)
